    ...
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Public')" class="grid">
                <flux:navlist.item
                    :href="route('open.user', request()->route('user'))"
                    :current="request()->routeIs('open.user')"
                    wire:navigate
                >
                    {{ __('Statuses') }}</flux:navlist.item>

                <flux:navlist.item
                    :href="route('open.archives', request()->route('user'))"
                    :current="request()->routeIs('open.archives')"
                    wire:navigate
                >
                    {{ __('Archives') }}</flux:navlist.item>
            </flux:navlist.group>
        </flux:navlist>

        <livewire:pages::open.account_list :user="request()->route('user')" />

        <livewire:pages::open.recents :user="request()->route('user')" />
